...
I'm pretty sure now that you understand why I said considering only a **single vector per word**, is beyond restrictive, becasue it **does not** take into account the **surrounding words context**! 

Therefore we need to build a framework by which, we can modify this mappings in a way that takes into account the meaning of surrounding words, right? 🌱

To do that, we need to first get familiar with the concept of **Inner Product**! 

**But wait... remember **I mentioned that within the concept of word2vec, we map each word to a vector of d-dimensional! Where each of those d-dimensional vectors are associated with a particular meaning/topic! Keep this in mind then go to next paragraph👇

<img src="./img/innerpro.png"> 

## Introducing the inner product of two codes
The idea of inner product is to take the dot product of two vectors! The purpose of inner product is to **quantify the relation/similarity between words**! So, let's look at the below example;
